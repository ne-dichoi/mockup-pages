...
| `BLOCKED_CONFLICT` | 병합 충돌(안전 복구됨) | 충돌 파일을 알리고, 같은 파일을 다른 사람이 바꿨음을 설명. **임의로 해결하지 말고** 담당 개발자에게 문의하도록 안내 |
| `BLOCKED_REMOTE_UPDATED` | 올리는 사이 남이 먼저 올림 | "잠시 후 다시 올리기(publish) 하겠습니다" 안내(사용자 확인 후 재시도 가능) |
