...
(function(){
  ...
  var BOOKS=[
    {title:"세 마리 토끼 잡는 초등 독서논술 A단계 세트", img:"assets/best_1.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"독해"},
    {title:"세 마리 토끼 잡는 초등 독서논술 A1", img:"assets/best_1.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"독해"},
    {title:"세 마리 토끼 잡는 초등 독서논술 A2", img:"assets/best_1.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"독해"},
    {title:"세 마리 토끼 잡는 초등 독서논술 A3", img:"assets/best_1.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"독해"},
    {title:"세 마리 토끼 잡는 초등 독서논술 A4", img:"assets/best_1.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"독해"},
    {title:"세 마리 토끼 잡는 초등 독서논술 A5", img:"assets/best_1.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"독해"},
    {title:"세 마리 토끼 잡는 초등 독서논술 B단계 세트", img:"assets/best_1.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"독해"},
    {title:"세 마리 토끼 잡는 초등 독서논술 B1", img:"assets/best_1.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"독해"},
    {title:"세 마리 토끼 잡는 초등 독서논술 B2", img:"assets/best_1.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"독해"},
    {title:"세 마리 토끼 잡는 초등 독서논술 B3", img:"assets/best_1.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"독해"},
    {title:"Phonics Code 1 : Student Book", img:"assets/best_1.png", url:"교재상세.html", now:"12,600원", was:"14,000원", tag:"Phonics"},
    {title:"능률 VOCA 어원편", img:"assets/best_2.png", url:"교재상세.html", now:"12,150원", was:"13,500원", tag:"어휘"},
    {title:"Bricks Reading 150 Level 1", img:"assets/best_3.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"Reading"},
    {title:"Grammar Zone 기본편 1", img:"assets/best_4.png", url:"교재상세.html", now:"12,600원", was:"14,000원", tag:"Grammar"},
    {title:"리딩튜터 입문", img:"assets/best_5.png", url:"교재상세.html", now:"11,700원", was:"13,000원", tag:"독해"},
    {title:"주니어 리딩튜터 1", img:"assets/best_2.png", url:"교재상세.html", now:"12,150원", was:"13,500원", tag:"독해"},
    {title:"중등 영어문법 3800제", img:"assets/best_3.png", url:"교재상세.html", now:"13,500원", was:"15,000원", tag:"문법"},
    {title:"워드마스터 중등 베이직", img:"assets/best_4.png", url:"교재상세.html", now:"10,800원", was:"12,000원", tag:"어휘"}
  ];
  ...
  function initSearch(box, opts){
    ...
    function renderAuto(q){
      ...
      /* 개선(UC 검색): 결과 행에서 상세 이동 없이 바로 담기 — 행 우측에 장바구니 아이콘 */
      var list=matches.map(function(b,i){
        var cart=opts.onSelect?'':'<button type="button" class="sa-row-cart" data-i="'+i+'" aria-label="장바구니 담기"><img src="assets/ic_cart_black.svg" alt=""></button>';
        return '<li class="it'+(i===0?' on':'')+'" data-i="'+i+'"><a href="'+b.url+'">'+esc(b.title)+'</a>'+cart+'</li>';
      }).join('');
      var first=matches[0];
      /* 미리보기: 표지 + 교재명 + 가격 + [장바구니][바로구매] — "이 책이 맞는지 확인 후 바로 도달" */
      var buyBox=opts.onSelect?'' :
        '<div class="sa-pv-price"><span class="now">'+esc(first.now||'')+'</span>'+(first.was?'<span class="was">'+esc(first.was)+'</span>':'')+'</div>'
        +'<div class="sa-pv-btns"><button type="button" class="sa-btn cart">장바구니</button><button type="button" class="sa-btn buy">바로구매</button></div>';
      pop.innerHTML='<div class="sa-wrap">'
        ...
        +'<p class="sa-pv-name">'+esc(first.title)+'</p>'+buyBox+'</div>'
        +'</div>';
      ...
      var pvNow=pop.querySelector('.sa-pv-price .now');
      var pvWas=pop.querySelector('.sa-pv-price .was');
      var curBook=first;
      function setPreview(b){ pvImg.src=b.img; pvName.textContent=b.title; if(pvNow) pvNow.textContent=b.now||''; if(pvWas) pvWas.textContent=b.was||''; curBook=b; }
      items.forEach(function(li){
        li.addEventListener('mouseenter',function(){
          items.forEach(function(x){ x.classList.remove('on'); });
          li.classList.add('on');
          setPreview(matches[+li.getAttribute('data-i')]);
        });
        if(opts.onSelect){
      ...
      if(opts.onSelect){
        ...
        /* 행 우측 장바구니 → 담기 확인 (상세 이동 없이 바로) */
        pop.querySelectorAll('.sa-row-cart').forEach(function(btn){
          btn.addEventListener('click',function(e){
            e.preventDefault(); e.stopPropagation();
            addToCart(matches[+btn.getAttribute('data-i')]);
          });
        });
        var cartBtn=pop.querySelector('.sa-btn.cart'), buyBtn=pop.querySelector('.sa-btn.buy');
        if(cartBtn) cartBtn.addEventListener('click',function(e){ e.preventDefault(); addToCart(curBook); });
        if(buyBtn) buyBtn.addEventListener('click',function(e){ e.preventDefault(); location.href='장바구니.html'; });
      }
    }

    /* 담기 성공 얼럿 → 장바구니 이동 확인 (와이어프레임 동작) */
    function addToCart(b){
      if(window.confirm('\''+b.title+'\'\n상품이 장바구니에 담겼습니다. 장바구니로 이동하시겠습니까?')){
        location.href='장바구니.html';
      }
    }
