...
(function(){
  ...
  /* ===== MY / 고객센터 드롭다운 — 터치기기(호버 불가) 대응 =====
     데스크톱(호버 지원)에서는 기존 :hover 동작을 그대로 두고, 터치기기에서만
     첫 탭에 드롭다운을 열고(.force-open) 링크 이동은 두 번째 탭부터 되게 한다. */
  (function(){
    var hoverCapable = window.matchMedia && window.matchMedia('(hover: hover)').matches;
    if (hoverCapable) return;
    var wraps=[].slice.call(document.querySelectorAll('.head-ic-wrap'));
    wraps.forEach(function(wrap){
      var link=wrap.querySelector('.head-ic');
      if(!link) return;
      link.addEventListener('click',function(e){
        if(wrap.classList.contains('force-open')) return; // 두 번째 탭 → 정상 이동
        e.preventDefault();
        wraps.forEach(function(w){ if(w!==wrap) w.classList.remove('force-open'); });
        wrap.classList.add('force-open');
      });
    });
    document.addEventListener('click',function(e){
      wraps.forEach(function(w){ if(!w.contains(e.target)) w.classList.remove('force-open'); });
    });
  })();
  ...
  /* ===== 전체메뉴(☰) 드로어 — 모바일/태블릿에서 GNB 카테고리 전체를 여닫이 패널로 노출 =====
     데스크톱 GNB(가로 메가메뉴)는 그대로 두고, 지금까지 클릭이 연결돼있지 않던
     ☰ 아이콘에 "전체 카테고리 목록" 패널을 새로 연결한다. */
  (function(){
    var menuBtn=gnb.querySelector('.menu'); if(!menuBtn) return;
    document.body.insertAdjacentHTML('beforeend',
      '<div class="mnav-backdrop" id="mnavBackdrop"></div>'+
      '<aside class="mnav" id="mnav"><div class="mnav-head"><span>전체메뉴</span>'+
      '<button type="button" class="mnav-close" aria-label="닫기">&times;</button></div>'+
      '<nav class="mnav-list" id="mnavList"></nav></aside>');
    var mnav=document.getElementById('mnav');
    var backdrop=document.getElementById('mnavBackdrop');
    var list=document.getElementById('mnavList');
    list.innerHTML=items.map(function(a){
      var name=a.textContent.trim();
      var subs=MENU[name];
      if(!subs) return '<a class="mnav-link" href="'+a.getAttribute('href')+'">'+name+'</a>';
      return '<div class="mnav-group"><button type="button" class="mnav-toggle">'+name+'<span class="car">&#9662;</span></button>'
        +'<div class="mnav-sub">'+subs.map(function(t){ return '<a href="리스트_교재구매.html">'+t+'</a>'; }).join('')+'</div></div>';
    }).join('');
    [].slice.call(list.querySelectorAll('.mnav-toggle')).forEach(function(btn){
      btn.addEventListener('click',function(){
        var grp=btn.closest('.mnav-group');
        var willOpen=!grp.classList.contains('open');
        [].slice.call(list.querySelectorAll('.mnav-group.open')).forEach(function(g){ g.classList.remove('open'); });
        if(willOpen) grp.classList.add('open');
      });
    });
    function openMnav(){ mnav.classList.add('open'); backdrop.classList.add('open'); }
    function closeMnav(){ mnav.classList.remove('open'); backdrop.classList.remove('open'); }
    menuBtn.addEventListener('click',function(e){ e.stopPropagation(); openMnav(); });
    backdrop.addEventListener('click',closeMnav);
    mnav.querySelector('.mnav-close').addEventListener('click',closeMnav);
