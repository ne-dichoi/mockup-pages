...
## 교훈: 반응형 검증은 "실제 인터랙션"까지 포함해야 한다 (2차 재작업 반영)

이전 파일럿은 **'오버플로 수치 0'** 지표만 보고 통과 처리했으나, 실사용에서 **햄버거 메뉴가 클릭해도 안 열려 모바일 메뉴 이동이 불가능**했다. 수치 지표는 레이아웃 잘림만 잡을 뿐, **기능 결함(핸들러 누락·CTA 클릭 불가·링크 미이동)은 못 잡는다.**

**검증은 반드시 실제 인터랙션까지 포함한다:**

1. **햄버거 클릭 → 메뉴 패널 실제 노출**: `classList.contains('open')` 같은 클래스 체크가 아니라, `getComputedStyle` + `getBoundingClientRect()`로 **패널이 실제로 화면에 보이는지(가시성·비제로 크기·좌표)**를 evaluate로 확인한다. `aria-expanded` 토글도 함께 검증.
2. **메뉴 항목 href 유효성**: 드로어 항목이 `#` 플레이스홀더가 아닌 **실제 페이지로 이동 가능**한지 확인. (본 목업은 GNB 1depth가 `#`였으므로, 드로어는 하위 항목을 실제 링크(`리스트_교재구매.html`)로 구성.)
3. **주요 CTA 가시성+클릭 가능**: `elementFromPoint`로 **다른 요소에 가려지지 않았는지**까지 확인(뷰포트 내에 있어도 오버레이에 덮이면 클릭 불가).
4. **한글 파일명**은 `encodeURIComponent`로 `page.goto`.

> 핵심: **"수치가 0"은 "기능이 동작한다"를 의미하지 않는다.** 클릭·이동·가시성을 실제로 재현하는 테스트만이 기능 결함을 잡는다.

### 2차 스코프(index·리스트 2페이지) 실측 결과

공통 헤더에 **모바일 전체메뉴 드로어**(`js/layout.js` + `css/layout.css`)를 구현. 대표 2페이지 × 3뷰포트(375·768·1440) 전수 통과:

| 페이지 | 뷰포트 | 잘림0 | 햄버거 동작 | 항목 href 유효 | CTA 가시·클릭 |
|---|---|---|---|---|---|
| index | 375 / 768 / 1440 | ✅ 0 | ✅ PASS | ✅ | ✅ |
| 리스트_교재구매 | 375 / 768 / 1440 | ✅ 0 | ✅ PASS | ✅ | ✅ |

- 햄버거는 **공통 헤더(layout.js 주입)**라 2페이지 검증으로 layout.js를 쓰는 20개 페이지 전체에 동일 적용된다.
- 리스트 페이지에서 드로어 링크 클릭 시 목적지가 현재 페이지(리스트_교재구매.html)와 동일해 URL이 안 바뀌는데, 이는 링크가 정상 동작하는 것이며(같은 페이지 재로드) 결함이 아니다. index에서는 다른 페이지로 이동해 URL 변경까지 확인됨.
