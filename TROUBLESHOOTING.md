...
## 페이지 인터랙션 전체 멈춤 (`Uncaught ReferenceError: xxx is not defined`)
- 증상: 배너 캐러셀·GNB 드롭다운·플로팅 메뉴 등 인라인 스크립트로 동작하는 인터랙션이 한꺼번에 안 된다. 콘솔에 `Uncaught ReferenceError: <변수> is not defined`.
- 원인: 인라인 `<script>`가 하나의 큰 IIFE 안에 여러 기능 블록을 담고 있어, 앞쪽 블록에서 던진 런타임 에러가 뒤쪽 블록(캐러셀 등) 실행을 통째로 중단시킨다. 카드 렌더링 코드를 고치면서 배열 정의(`var covers=[...]`)를 지웠는데 다른 곳에서 그 변수를 계속 참조해 발생했다.
- 해결: 참조가 남지 않게 정의를 되살리거나 참조부를 새 데이터로 함께 교체한다. `node --check`는 문법만 보고 이런 런타임 참조 오류는 못 잡으니, 브라우저 콘솔(F12) 에러로 확인한다.
- 관련: 인라인 스크립트 편집 / 브라우저 콘솔
